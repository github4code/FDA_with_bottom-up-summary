    ...
    for idx, doc in tqdm(enumerate(documents)):
        ...
        if comb is None:
            if doc is None:
                ...
                writer.write(" ".join(doc.doc_sents) + '\n')  # keep origin artilce
            # writer.write('\n')
        else:
            if comb[0] is None:
                writer.write(" ".join(doc.doc_sents) + '\n')  # keep origin artilce
            else:
                writer.write(" ".join([doc.doc_sents[i] for i in comb[0]]) + '\n')  # keep best sentences
...
def load_data(src_file, tgt_file):
    ...
    with open(src_file, 'r', encoding='utf-8') as src_reader, \
        ...
        for src_line, tgt_line in tqdm(zip(src_reader, tgt_reader)):
            ...
            index_file.write(str(len(src_sents))+'\n')
